import sqlite3
from Check import assertion_is_primary_key_valid
...
class SQLiteDatabase:
    ...
    def create_table_with_primary_key(self, table_name, primary_keys_list=None, *args):
        if assertion_is_primary_key_valid(primary_keys_list, *args):
            arguments = list(args)

            if primary_keys_list is not None:
                primary_keys = ["PRIMARY KEY (" + str(key) + ")" for key in primary_keys_list]
                arguments.extend(primary_keys)

            arguments = ",".join(arguments)

            # query = "CREATE TABLE IF NOT EXISTS {} ({});".format(table_name, arguments)
            self.create_table(table_name, arguments)
    ...
    def get_rows_referential_integrity(self, table_names, row_list=None, foreign_key_list=None):
        ...
        arguments = "WHERE "
        count = 0
        if foreign_key_list is not None:
            for condition in foreign_key_list:
                if count == 0:
                    arguments += condition[0] + " " + condition[1] + " " + condition[2]
                else:
                    arguments += " AND " + condition[0] + " " + condition[1] + " " + condition[2]
                count += 1
    ...
    def set_foreign_key_pragma(self, value=True):
        if value == True:
    ...
    def get_distinct_values(self, table, columns=None):
        if columns is None:
            return None

        value_list = ", ".join(columns)

        query = "SELECT {} FROM {};".format(value_list, table)
        self.cursor.execute(query)
        row_list = self.cursor.fetchall()
        self.connection.commit()
        return row_list
