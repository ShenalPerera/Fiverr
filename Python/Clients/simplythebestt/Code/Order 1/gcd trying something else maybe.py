terminate = False

# Do computation until user want to terminate
while not terminate:
    n = eval(input("Enter the number (n): "))
    if n <= 0:
        print("The number entered as n should be positive whole number!")
        continue
    m = eval(input("Enter the number (m): "))
    if m <= 0:
        print("The number entered as m should be positive whole number!")
    if n <= 0 or m <= 0:
        continue

    # swap the values if n is less than m
    if n < m:
        temp = n
        n = m
        m = temp

    # store the numbers to show to users
    num1 = n
    num2 = m

    # Euclid's Algorithm
    while m != 0:
        temp = m
        m = n % m
        n = temp

    # print the result to the user
    print(f"GCD({num1},{num2}) is {n}")

    answer = input("Do you want to another computation: ")
    answer = answer.lower()

    if answer == "n" or answer == "no":
        terminate = True
        print("Goodbye!")
    elif answer == 'y' or answer == "yes":
        terminate = False
    else:
        print("Invalid input!")
        break
