...
int main() {
    ...
    FILE * fp = fopen("cop3502-as4-output.txt","w");
...
void inorder_item_tree(item_node * parent,FILE * fp){
    if(parent->left) inorder_item_tree(parent->left,fp);

    print_item_tree_node(parent,fp);

    if (parent->right) inorder_item_tree(parent->right,fp);
}

void inorder_name_tree(tree_name_node * parent, FILE * fp){
    if(parent->left) inorder_name_tree(parent->left,fp);
    print_name_tree_node(parent,fp);
    if (parent->right) inorder_name_tree(parent->right,fp);
}
...
int count_items(item_node * t ){
    ...
    if(t->left) val += count_items(t->left);
    if(t->right) val += count_items(t->right);
