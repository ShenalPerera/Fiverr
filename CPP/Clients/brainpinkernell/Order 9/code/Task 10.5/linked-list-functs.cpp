...
void print_list(Node* head_ptr)
{
    ...
    while (curr_node_ptr != NULL)
    {
        ...
        cout << curr_node_ptr->get_data().to_string() << endl;
...
bool remove_instance(Node *& head_ptr, NodeDataType remove_data){
    ...
    if (current != NULL && current->get_data() == remove_data){
        head_ptr = current->get_next();
        delete current;
        return true;
    }

    else{
        while (current != NULL && current->get_data() != remove_data){
            prev = current;
