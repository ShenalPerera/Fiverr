...
class Plane {
	...
	getSeats() {
		return Seat**
		//include constant keyword at the end?
	}
	getAssignSeat(Plane, string, string) {
		return true;
	}
}
...
//***
setSeats() {
	seat = new Seat*[numOfRows];
	for (i = 0; i < 1; numOfRows ++) {
		seat = new Seat[numOfLetters]
	}
}
...
//***       operator. See Chapter 9.6.
//***
assignSeat(plane, seat, passengerName) {
	assignSeatOk = false;
	upperCase(seat)
