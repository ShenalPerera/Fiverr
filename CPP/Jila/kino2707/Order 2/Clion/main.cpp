...
int main() {
    ...
    // create player and the map using the text files
    ...
    // Loop until user will exit from the game
    do {
        // get the current location adn show the caution message
        ...
        // display the attributes of the players
    ...
    // show again attributes when aa round is completed
    player.displayAttributes();
...
void  displayRules(string filename){
    ...
    // crashed message show when the file is missing
    if (fin.fail()){
...
Player createPlayer(string name){
    ...
    // set the default values to the user attributes
...
// set the next desitnaations of each location of the map
void setDestinations(int location1,int location2, int location3, Location &current, vector<Location> &map){
    // get the current location and add the next destinations by location inddex
...
void assignMonstersAndItems(vector<Location> &map,Monster monsters_map[],Item items_map[]){
    ...
    for (int j = 0; j < 14; ++j) {
        ...
        // set the random mosters and random items
    ...
    // assign the artifact to the map with high risk monster location probability
    int count = 4;
    for (int j = 1; j < 20; ++j) {
    ...
    // if any artifacts left assign them into the map
    while (count > 0 ){
...
void move(vector<Location> &map,Player &player){
    ...
    for (int i = 0; i < 20; ++i) {
        if (map.at(i).getHasPlayer()){
            ...
            int path;
            cin >> path;
            // get the index of the path and set the user
...
void attack(vector<Location> & map,Monster monsters[],Player &player){
    ...
    // block the attack move if monster is not in the cave
    if (!location->getHasMonster()){
        cout << "Cannot attack without a monster" << endl;
        return;
    }

    // if not get the monster in the current location
    Monster * monster = &monsters[location->getIndex()];
    monster->getDetails();  // show the monster details
    cout << endl;
    // get the inventory values
    bool sword = false, armour= false, axe= false,chain= false,boots= false;
    for (int i = 0; i < player.getInventory().size(); ++i) {
        string item_name = player.getInventory().at(i).getName();
       // set the boolean values to true if they presents in the inventory
        if (item_name == "Sword") {
            sword = true;
        }
        if (item_name == "Armour") {
    ...
    while (true){
        ...
        // calculate the attack and damasges and update the attributes
